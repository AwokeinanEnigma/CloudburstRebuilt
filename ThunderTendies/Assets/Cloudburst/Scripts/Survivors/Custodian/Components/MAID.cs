...
namespace Cloudburst.Custodian.Components
{
    class MAID : MonoBehaviour
    {
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (component && controller && PassesFilter(controller.teamFilter) && !controller.gameObject.HasComponent<ProjectileSlow>())
            {
                ...
                var cing = controller.AddComponent<ProjectileSlow>();
                cing.maxVelocityMagnitude = 3;
                cing.antiGravity = 1;

                var effect = controller.AddComponent<ProjectileEffectManager>();
                effect.effect = AssetLoader.mainAssetBundle.LoadAsset<GameObject>("MAIDTriggerEffect");
            }
        }
        ...
        public void FixedUpdate()
        {
            ...
            {
                CCUtilities.LogI("pause & trigger");
                triggered = true;
                pause = true;
