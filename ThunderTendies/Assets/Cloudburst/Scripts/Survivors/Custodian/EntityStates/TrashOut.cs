    ...
    public class TrashOut : BaseSkillState
    {
        ...
        public override void OnEnter()
        {
            ...
            if (base.isAuthority)
            {
                ...
                //CCUtilities.LogI("Stage: " + stage.ToString());
            }
        }

        public void SetHookReference(GameObject winch)
        {
            //CCUtilities.LogI("got winch!");
            _winch = winch;
        }
                ...
                if (base.isAuthority)
                {
                    if (this.target)
                    {
                        ...
                        if (Vector3.Distance(base.transform.position, target.transform.position) < distance + 5f && target)
                        {
                            ...
                            {
                                CCUtilities.LogI($"Check1: {NetworkServer.active}"); //
                                ...
                                {
                                    CCUtilities.LogI($"Check2: {NetworkServer.active}");
                                ...
                                {
                                    CCUtilities.LogI($"Check3: {NetworkServer.active}");
                                    SpikingComponent based = target.healthComponent.AddComponent<SpikingComponent>();
                                    based.interval = 1.5f;
                                    based.originalSpiker = this.gameObject;
                                }
