...
public static class CCUtilities
{
    ...
    public static bool RegisterNewProjectile(GameObject projectileObject)
    {
        ...
        if (projectileObject.HasComponent<ProjectileController>())
        {
            ...
            LogD("Registered projectile " + projectileObject.name + " to the projectile catalog!");
            return true;
        }
        LogF("FATAL ERROR:" + projectileObject.name + " failed to register to the projectile catalog!");
        return false;
    }
    /// <summary>
    ...
    public static bool RegisterNewBody(GameObject bodyObject)
    {
        if (bodyObject)
        {
            ...
            LogD("Registered body " + bodyObject.name + " to the body catalog!");
            return true;
        }
        LogF("FATAL ERROR:" + bodyObject.name + " failed to register to the body catalog!");
        return false;
    }
    /// <summary>
    ...
    public static bool RegisterNewMaster(GameObject master)
    {
        if (master && master.HasComponent<CharacterMaster>())
        {
            ...
            LogD("Registered master " + master.name + " to the master catalog!");
            return true;
        }
        LogF("FATAL ERROR: " + master.name + " failed to register to the master catalog!");
        return false;
    }
    /// <summary>
    ...
    //this is a shit fix
    public static string ConvertResourceLoadToGUID(string FUCK)
    {
        string cum = "";
        if (Addressable2Path.oldResourcesPathToGuid.TryGetValue(FUCK, out cum))
        {
            LogD($"Attempting to find addressable key for: {FUCK}. Retreived key: {cum}");
            return cum;
        };
        if (Addressable2Path.oldResourcesPathToGuid.ContainsValue(FUCK))
        {
            return FUCK;
        }
    ...
    public static void CreateEmptySkills(GameObject survivor)
    {
        ...
        else LogF("Tried to create empty skills on a null GameObject!");
    }
    /// <summary>
